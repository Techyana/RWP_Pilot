...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  const handleMarkOne = (id: string) => {
    markOneAsRead(id)
    // keep panel open so user can read others
  }
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        ...
        <div className="relative" ref={panelRef}>
          ...
          {isPanelOpen && (
            <div
              className="
                fixed inset-x-0 top-16 mt-2
                sm:absolute sm:inset-auto sm:right-0 sm:top-auto sm:mt-2
                w-80 max-w-[95vw] mx-auto sm:mx-0 z-30
            ...
            >
              <div className="p-3 flex justify-between items-center border-b dark:border-gray-700">
                ...
                <button
                  ...
                  className="text-xs text-brand-primary hover:underline disabled:text-gray-400"
                >
                  Mark all as read
                </button>
              </div>
                  ...
                  notifications.map((n) => (
                    ...
                    >
                      <div className="flex items-start space-x-3">
                        {!n.isRead && (
                          <div className="h-2 w-2 rounded-full bg-brand-primary mt-1.5 flex-shrink-0" />
                        )}
        ...
        {/* Divider */}
        <div className="hidden sm:block border-l border-gray-200 dark:border-gray-700 h-8" />

        {/* User Info */}
        <div className="text-right">
          <p className="font-semibold text-gray-900 dark:text-white truncate max-w-28 sm:max-w-none">
            {user?.name} {user?.surname}
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 capitalize hidden sm:block">
